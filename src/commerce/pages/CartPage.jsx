...
function productImage(product) {
  ...
  return images.find((image) => image.isPrimary)?.url || images[0]?.url || product.image?.url || product.image || '';
}
function EmptyCart() {
  ...
  return <div className="empty-cart-layout">
    ...
    {suggestions.length > 0 && <section className="cart-suggestions" aria-labelledby="cart-suggestions-title"><h2 id="cart-suggestions-title">You might like</h2><div>{suggestions.map((product) => <Link to={`/products/${product.id}`} key={product.id}><ProductImage src={productImage(product)} name={product.name} /><strong>{product.name}</strong><span>{money(product.price, 'NGN')}</span></Link>)}</div></section>}
  </div>;
}
...
export default function CartPage() {
  ...
  const change = async (mutation, input) => { try { await mutation.run(input); } catch { /* Inline error. */ } };
  return <>
    ...
    <QueryState query={query}>
      {items.length === 0 ? <EmptyCart /> : <div className="commerce-grid commerce-grid--cart">
        <Card title={`Your cart (${cartCount(query.data)} items)`} icon={ShoppingCart}>
          ...
          {confirmClear && <div className="commerce-note"><p>Remove all items from your cart?</p><div className="commerce-actions"><button className="commerce-button commerce-button--secondary" disabled={busy} onClick={async () => { await change(clear); setConfirmClear(false); }}>Yes, clear cart</button><button className="commerce-text-button" onClick={() => setConfirmClear(false)}>Keep items</button></div></div>}
        </Card>
