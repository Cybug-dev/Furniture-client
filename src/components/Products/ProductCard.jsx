...
function primaryImage(product) {
  ...
  const preferred = images.find((image) => image.isPrimary)
...
export default function ProductCard({ product, onNotice }) {
  ...
  const hasDiscount = Number.isFinite(currentPrice) && Number.isFinite(originalPrice) && originalPrice > currentPrice;
  const productPath = `/products/${encodeURIComponent(product.id)}`;

  const addToCart = async () => {
    try {
      const result = await cart.add(product.id, 1);
      if (result) onNotice?.({ type: 'success', message: `${product.name} was added to your cart.` });
    } catch (error) {
      onNotice?.({ type: 'error', message: error.message || 'This item could not be added to your cart.' });
    }
  };
    ...
    <article className="products-card">
      <button
        ...
        onClick={() => {
          const next = !liked;
          setLiked(next);
          onNotice?.({ type: 'info', message: next ? `${product.name} was liked.` : `${product.name} was unliked.` });
        }}
      >
        <Heart size={16} fill={liked ? 'currentColor' : 'none'} aria-hidden="true" />
      </button>
      <Link className="products-card__details" to={productPath} aria-label={`View details for ${product.name}`}>
        <div className="products-card__media">
          {hasDiscount && <span className="products-card__badge">Sale</span>}
          {src && !imageFailed ? (
            <img src={src} alt={product.name} loading="lazy" decoding="async" onError={() => setImageFailed(true)} />
          ) : (
            <div className="products-card__placeholder" role="img" aria-label={`${product.name} image unavailable`} />
          )}
        </div>
      ...
      <button className="products-card__cart" type="button" disabled={cart.isPending || product.stockQuantity === 0} onClick={addToCart}>
        <ShoppingCart size={16} aria-hidden="true" />
        {product.stockQuantity === 0 ? 'Out of stock' : cart.isPending ? 'Adding…' : 'Add to Cart'}
      </button>
    </article>
  );
}
